...
function renderPulseFeed() {
    ...
    if (!document.getElementById('pulseFilters')) {
        ...
        filterBar.className = 'flex justify-center gap-4 mb-6 -mt-8';
        filterBar.innerHTML = `<button onclick="toggleSniperMode()" id="sniperBtn" class="px-6 py-2 rounded-full border border-slate-200 text-[10px] uppercase tracking-widest transition-all ${window.isSniperMode ? 'bg-indigo-600 text-white hover:brightness-75 border-indigo-600' : 'text-slate-400 hover:bg-slate-50 hover:text-indigo-600'}">Sniper Mode (High Impact)</button>`;
    ...
    Object.keys(autoHistory).forEach(id => {
        ...
        const petData = (window.PETS || PETS || []).find(p => p.id == id);
        ...
        for (let i = petHistory.length - 1; i > 0; i--) {
            ...
            if (current.value !== previous.value) {
                ...
                // Prevent NaN if previous value was 0 or O/C
                const pct = prevVal > 0 ? Math.abs(((curVal - prevVal) / prevVal) * 100) : 0;
                ...
                changes.push({
                    ...
                    isRising: isRising,
                    percentChange: pct,
                    timestamp: current.timestamp || new Date(current.date).getTime()
                });
            }
        }
    });
    ...
    feed.innerHTML = displayChanges.map((change, index) => `
        <div class="reveal p-6 bg-white border border-slate-100 rounded-[2.5rem] hover-lift flex flex-col md:flex-row items-center gap-6" style="transition-delay: ${index * 50}ms">
            ...
            <div class="flex-grow text-center md:text-left">
                ...
                <p class="pulse-time text-slate-400 text-xs font-normal" data-timestamp="${change.timestamp}">${formatRelativeTime(change.timestamp)}</p>
            </div>
            ...
            <button onclick="openDetails(${change.pet.id})" class="px-6 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] uppercase tracking-widest font-normal hover:brightness-110 active:scale-95 transition-all">View Analytics</button>
        </div>
    `).join('');
...
window.toggleSniperMode = function() {
    ...
    btn.classList.toggle('hover:brightness-75', window.isSniperMode);
    btn.classList.toggle('text-slate-400', !window.isSniperMode);
    btn.classList.toggle('hover:bg-slate-50', !window.isSniperMode);
    btn.classList.toggle('hover:text-indigo-600', !window.isSniperMode);
...
function startPulseTimer() {
    ...
    window.pulseInterval = setInterval(() => {
        document.querySelectorAll('.pulse-time').forEach(el => {
